...
## 목차
- [프로젝트 목적](#프로젝트-목적) <br/>
- [팀원 소개 및 역할 분담](#팀원-소개-및-역할-분담) <br/>
- [개발 환경](#개발-환경) <br/>
...
- 로그인 페이지
- 게시물 업로드 페이지
- 게시물 상세 페이지 중 댓글 등록, 수정, 삭제 기능 구현
- 버튼 공통 컴포넌트
- 회의록 작성
- 리드미 작성
- 발표
...
## 개선 사항
- 추후 확장 및 유지 보수를 고려해 `재사용되는 컴포넌트`는 별도로 common 폴더에서 관리할 수 있도록 하였습니다.
- 효율적인 작업상황을 고려하여 `반복되는 함수`는 utils 폴더로 따로 분리하여 사용했습니다.
- 이후 리팩토링시 진행 예정인 다크모드, 반응형 작업을 위해 `변수컬러와, rem 단위`를 사용하였습니다.
- 사용자 경험을 고려하여 Footer를 통해 바로 접근하기 어려운 페이지를 라우팅 하였습니다.
- `로딩 애니메이션`을 추가하여 사용자가 로딩 상태를 인지할 수 있도록 했습니다.
- `useParams`를 이용해 사용자의 아이디를 받아와 정보를 띄워주도록 하였고, 내 프로필에서는 상품 등록 및 프로필 수정 페이지로 이동할 수 있는 버튼이 나타나고, 다른 유저의 프로필에서는 팔로우 버튼이 나타나도록 하였습니다.
- 댓글 모달 클릭시 내 댓글이면 삭제, 다른 유저의 댓글이면 신고 기능을 사용할 수 있도록 하였습니다.
<br/>
